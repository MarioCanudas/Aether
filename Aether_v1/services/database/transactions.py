...
class TransactionsDBService(BaseDBService):
    ...
    def get_transactions(
        ...
        **conditions: Any,
        ...
        if show_categories_names:
            query += (
                f" LEFT JOIN categories ON t.{self.category_id} = categories.{self.category_id}"
            )
        ...
        if statement_type:
            query += " AND statement_type = %(statement_type)s"
            params["statement_type"] = statement_type.value

        if amount_types:
            query += (
                f" AND type IN ({', '.join([f'%(type_{i})s' for i, _ in enumerate(amount_types)])})"
            )
    ...
    def get_transactions_period(self, user_id: int) -> Period:
        ...
        result = self.execute_query(
            query, params={"user_id": user_id}, fetch="one", dict_cursor=True
        )
    ...
    async def get_specific_period_sums(
        ...
        return FinancialAmountsSums(
            income=Decimal("0.00"), withdrawal=Decimal("0.00"), savings=None
        )
