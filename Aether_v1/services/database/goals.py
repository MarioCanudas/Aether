...
class GoalsDBService(BaseDBService):
    ...
    def add_goal(self, goal: Goal) -> None:
        with self.transaction():
            query = """
                INSERT INTO goals (user_id, type, category_id, name, amount, added_amount, created_at, updated_at, start_date, end_date)
                VALUES (%(user_id)s, %(type)s, %(category_id)s, %(name)s, %(amount)s, %(added_amount)s, %(created_at)s, %(updated_at)s, %(start_date)s, %(end_date)s)
            """
