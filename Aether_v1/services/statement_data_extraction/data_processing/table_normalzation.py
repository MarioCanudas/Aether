...
class DateNormalizer(ColumnNormalizer):
    @staticmethod
    def normalize_date_with_year(
        date: str, date_pattern: str, groups_date: DateGroups, month_pattern: dict[str, str]
    ) -> str:
        """
    ...
    @staticmethod
    def normalize_date_without_year(
        date: str,
        years: list[int],
        date_pattern: str,
        groups_date: DateGroups,
    ...
    def normalize_column(
        self,
        reconstructed_table: ReconstructedTable | Any,
        bank_properties: BankProperties | Any = None,
        years: list[int] | Any = None,
    ) -> pd.Series:
        """
        ...
        if date_groups.has_year:
            ...
            dates = date_column.apply(
                lambda x: self.normalize_date_without_year(
                    x, years, date_pattern, date_groups, month_pattern
                )
            )
...
class AmountNormalizer(ColumnNormalizer):
    ...
    def normalize_amount_for_multiple_columns(row: Any, amount_columns: AmountColumns) -> float:
        ...
        # Handle balance column when both income/expense are empty
            ...
            amount_columns.has_balance
            and is_income_effectively_empty
            and is_expense_effectively_empty
        ):
    ...
    def normalize_column(
        self,
        reconstructed_table: ReconstructedTable | Any,
        amount_columns: AmountColumns | Any = None,
        amount_signs: AmountSigns | Any = None,
    ) -> pd.Series:
        """
...
class DefaultTableNormalizer(TableNormalizer):
    ...
    def normalize_table(self, years: list[int], filename: str) -> TransactionsTable:
        ...
        if isinstance(amount_normalizer, AmountNormalizer):
            normalized_table.amounts = amount_normalizer.normalize_column(
                reconstructed_table, amount_columns, amount_signs
            )
        ...
        if self.bank_properties.special_data_filtering is not None:
            normalized_table = self.bank_properties.special_data_filtering.filter_special_data(
                normalized_table
            )
