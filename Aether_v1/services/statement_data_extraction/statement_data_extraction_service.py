...
from .document_processing import DefaultDocumentAnalyzer, DefaultTextProcessor, PDFReader
...
class StatementDataExtractionService:
    ...
    def get_reconstructed_table(self) -> ReconstructedTable:
        ...
        filtered_table_words = corrected_extracted_words.filter_table_by_phrases(
        ...
        reconstructed_table = TableReconstructor(
            grouped_rows, column_delimitations, bank_properties
        )
    ...
    def get_normalized_table(self) -> TransactionsTable:
        ...
        table_normalizer = DefaultTableNormalizer(
            reconstructed_table, bank_properties, (date_normalizer, amount_normalizer)
        )
    ...
    def get_transactions(self) -> TransactionsTable:
        ...
        initial_balance_row = self.metadata_extractor.get_initial_balance_row(
            self.user_id, self.filename
        )
        generated_amount_row = self.metadata_extractor.get_generated_amount_row(
            self.user_id, self.filename
        )
