...
class DefaultDocumentAnalyzer(DocumentAnalyzer):
    ...
    def detect_bank_in_footer(self) -> BankName | None:
        ...
        try:
            ...
            document_height = (
                self.reader.get_height()
            )  # Get height to determine the footer threshold

            footer_percentage = 0.15
            footer_threshold = (
                document_height * footer_percentage
            )  # Calculate the footer threshold using 15% of the document height
            ...
            for bank in BankName:
                ...
                mask = footer_text.str.contains(
                    f"\\b{re.escape(bank.value)}\\b", regex=True, na=False
                )
            ...
            if search_phrase_in_df(
                extracted_words.df, ["nu", "méxico", "financiera,"], type_return="bool"
            ):
                return BankName.NU
    ...
    def detect_bank_by_code(self) -> BankName | str | None:
        ...
        try:
            ...
            # na=False ensures that NaN values are treated as False instead of NaN
            mask = extracted_words.df["text"].str.contains(
                clabe_keyword, regex=True, case=False, na=False
            )
    ...
    def _is_new_credit_format(self, bank_properties: BankProperties) -> bool:
        ...
        try:
            ...
            start_phrase_found = search_phrase_in_df(
                extracted_words.df, start_phrase, type_return="bool"
            )
            end_phrase_found = search_phrase_in_df(
                extracted_words.df, end_phrase, type_return="bool"
            )
    ...
    def get_bank_properties(self) -> BankProperties:
        ...
        try:
            ...
            if statement_type == StatementType.CREDIT:
                ...
                old_properties = self.bank_properties_factory.get_bank_properties(
                    bank, statement_type, new_format=False
                )

                if old_properties and self._is_new_credit_format(old_properties):
                    # Try to get new format properties
                    new_properties = self.bank_properties_factory.get_bank_properties(
                        bank, statement_type, new_format=True
                    )
                    if new_properties:
                ...
                properties = self.bank_properties_factory.get_bank_properties(
                    bank, statement_type, new_format=None
                )
            ...
            raise ValueError(
                f"Error getting statement properties for bank {bank} and statement type {statement_type}: {e}"
            )
