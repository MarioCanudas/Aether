...
class TransactionDefaultValues(BaseModel):
    ...
    def from_dict(cls, values: dict[str, Any]) -> "TransactionDefaultValues":
        ...
        if (
            ...
            and not isinstance(values["amount"], Decimal)
        ):
            values["amount"] = Decimal(str(values["amount"]))
        if (
            "transaction_date" in values
            and values["transaction_date"] is not None
            and not isinstance(values["transaction_date"], date)
        ):
            values["transaction_date"] = date.fromisoformat(values["transaction_date"])
        if (
            "category_id" in values
            and values["category_id"] is not None
            and not isinstance(values["category_id"], int)
        ):
            values["category_id"] = int(values["category_id"])
        if (
            "description" in values
            and values["description"] is not None
            and not isinstance(values["description"], str)
        ):
...
class GoalDefaultValues(BaseModel):
    ...
    def from_dict(cls, values: dict[str, Any]) -> "GoalDefaultValues":
        ...
        if (
            ...
            values["amount"] = Decimal(str(values["amount"]))
        if (
            "period_range" in values
            and values["period_range"] is not None
            and not isinstance(values["period_range"], PeriodRange)
        ):
