...
def show_cards():
    # Page config
    st.set_page_config(page_title="Cards", page_icon=CARDS_ICON, layout="wide")
    ...
    if card_to_view:
        ...
        with left1:
            st.metric(
                label="Total income", value=give_amount_format(view_data.metrics.total_income)
            )

        with center1:
            st.metric(
                label="Total expenses", value=give_amount_format(view_data.metrics.total_expenses)
            )

        with right1:
            st.metric(
                label="Total balance", value=give_amount_format(view_data.metrics.total_balance)
            )
        ...
        with left2:
            st.altair_chart(
                view_data.income_vs_expenses_chart
            ) if view_data.income_vs_expenses_chart else st.info(
                "No data available", icon=":material/info:"
            )
