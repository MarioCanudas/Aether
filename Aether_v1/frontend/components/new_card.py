...
def new_card_popup():
    ...
    with st.form(key= 'new_card_form', border= False, clear_on_submit= True):
        ...
        card_bank = left.selectbox('Bank', options= BankName.get_values(), index= None, key= 'new_card_bank')
        card_statement_type = right.selectbox('Statement Type', options= StatementType.get_values(), index= None, key= 'new_card_statement_type')
