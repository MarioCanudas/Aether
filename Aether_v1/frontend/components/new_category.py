...
def new_category_popup():
    with st.form(key="new_category_form", border=False, clear_on_submit=True):
        ...
        new_category_group = st.selectbox(
            "Group", options=CategoryGroup.get_values(), index=None, key="new_category_group"
        )
        ...
        if st.form_submit_button("Add category", type="primary"):
            try:
                ...
                st.warning("Some field is invalid, try again", icon=":material/warning:")
            ...
            try:
                ...
                st.warning("Category already exists, try again", icon=":material/warning:")
