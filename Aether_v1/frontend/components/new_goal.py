...
def new_goal_popup():
    ...
    template_name = st.pills(
        "Template", options=list(templates_names.keys()), default=None, key="new_goal_template"
    )
    ...
    with st.form(key="new_goal_form", border=False, clear_on_submit=True):
        if template_name is not None:
            ...
            name = st.text_input(
                "Name",
                value=default_values.name if default_values.name else None,
                key="new_goal_name",
            )
            ...
            goal_type = left.selectbox(
                "Type",
                goal_types,
                index=goal_types.index(default_values.type.value) if default_values.type else None,
                key="new_goal_type",
            )

            category = right.selectbox(
                "Category", controller.get_categories(), index=None, key="new_goal_category"
            )
            ...
            goal_type = left.selectbox(
                "Type", controller.get_goal_types(), index=0, key="new_goal_type"
            )

            category = right.selectbox(
                "Category", controller.get_categories(), index=None, key="new_budget_category"
            )
        ...
        if period_range != PeriodRange.OTHER.value:
            ...
            end_date = right.date_input(
                "End date",
                value=controller.get_end_date(start_date, PeriodRange(period_range)),
                key="new_budget_end_date",
            )
        else:
...
def add_amount_popup(goal_id: int) -> None:
    ...
    if st.button(
        "Add amount", type="primary", key=f"add_amount_add_button_{goal_id}", disabled=amount <= 0
    ):
