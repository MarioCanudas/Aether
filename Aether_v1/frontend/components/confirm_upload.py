...
def confirm_upload_popup(transactions: list[Transaction]) -> None:
    ...
    if len(filtered_transactions_result.clean) > 0:
        ...
        st.dataframe(controller.transactions_to_df(filtered_transactions_result.clean, to_view= True))
    
    if len(filtered_transactions_result.potential_duplicates_to_upload) > 0:
        st.subheader('Potential duplicates to upload')
        st.dataframe(controller.transactions_to_df(filtered_transactions_result.potential_duplicates_to_upload, to_view= True))
    ...
    if len(filtered_transactions_result.duplicated) > 0:
        ...
        st.dataframe(controller.transactions_to_df(filtered_transactions_result.duplicated, to_view= True))
